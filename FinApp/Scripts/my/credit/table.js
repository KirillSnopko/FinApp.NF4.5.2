$(document).ready(function () {
    ...
    $(window).on('load', function () {
        $.get("/Credit/List", {},
            function (credits) {
                var table = $('<table class="table"><tr><th>Name</th><th>Returned</th><th>Common value</th><th>Open date</th><th>Closed date</th><th>Status</th><th>Options</th></tr>');
                $(credits).each(function (index, item) {
                    ...
                    if (returned < balanceOwed) {
                        table.append('<tr class="table-default"><td>' + info + '</td><td>' + returned + '</td><td><strong>' + balanceOwed + '</strong></td > <td>' + date1 + '</td><td>' + date2 + '</td><td>' + getProgress(balanceOwed, returned) + '</td><td>' + button1(id) + '</td></tr >');
                    } else {
                        table.append('<tr class="table-success"><td>' + info + '</td><td>' + returned + '</td><td><strong>' + balanceOwed + '</strong></td > <td>' + date1 + '</td><td>' + date2 + '</td><td>' + getProgress(balanceOwed, returned) + '</td><td>' + button2(id) + '</td></tr >');
                    }
                });
                table.append('</table>');
                $('#credits').html(table);
